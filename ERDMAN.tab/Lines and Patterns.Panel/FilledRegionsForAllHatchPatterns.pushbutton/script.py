...
def create_filled_region_types_for_fill_patterns(doc, fill_pattern_elements, filled_region_dict):
    errors = []
    ...
    try:
        ...
        for fill_pattern in fill_pattern_elements:
            if fill_pattern.Id not in existing_fg_pattern_ids: #if a fill pattern is not in a filled region, we need to make one.
                ...
                random_filled_region = FilteredElementCollector(doc).OfClass(FilledRegionType).FirstElement() # get any filled region type to prep for duplicaiton
                new_region           = random_filled_region.Duplicate(str(sanitize_name(fill_pattern.Name))) # have to duplicate the filled region. Setting filled region name to fill pattern name.
...
def main(doc):
    ...
    if form.ShowDialog() == DialogResult.OK:
        ...
        try:
            ...
            if user_input["create_filled_region_types_for_fill_patterns"]: 
                print("\nCreating Filled Regions...")
                create_filled_region_types_for_fill_patterns(doc, fill_pattern_elements, filled_region_dict)

            print("\nRefreshing Filled Region List...")
            filled_region_types, filled_region_dict = get_filled_region_types(doc, fill_pattern_dict)

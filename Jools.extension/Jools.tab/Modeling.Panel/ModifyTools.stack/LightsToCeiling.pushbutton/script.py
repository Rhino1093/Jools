...
def find_bottom_face_elevation(intersector, ref_ctx):
    """
    Given a hit on a ceiling (top or bottom), finds the lowest face 
    of that specific ceiling at the same XY coordinates.
    """
    ...
    # Cast a ray from 10 feet below the hit point UP.
    # We filter for the same element ID to find its bottom face.
    search_start = XYZ(hit_pt.X, hit_pt.Y, hit_pt.Z - 10.0)
    all_hits = intersector.FindAll(search_start, XYZ.BasisZ)
    
    min_z = None
    for h_ctx in all_hits:
        h_ref = h_ctx.GetReference()
        # Check if it's the same element (handling links)
        if h_ref.ElementId == el_id and h_ref.LinkedElementId == linked_id:
            h_z = h_ref.GlobalPoint.Z
            if min_z is None or h_z < min_z:
                min_z = h_z
                
    return min_z if min_z is not None else hit_pt.Z
    ...
    try:
        ...
        for i, el in enumerate(elements_to_process):
            ...
            try:
                ...
                # Cast Rays to find nearest ceiling
                ref_up = intersector.FindNearest(location_pt, XYZ.BasisZ)
                ref_down = intersector.FindNearest(location_pt, XYZ.BasisZ.Negate())
                
                dist_up = ref_up.GetReference().GlobalPoint.DistanceTo(location_pt) if ref_up else float('inf')
                dist_down = ref_down.GetReference().GlobalPoint.DistanceTo(location_pt) if ref_down else float('inf')

                # Identify the best reference context
                best_ref_ctx = None
                if ref_up and ref_down:
                    best_ref_ctx = ref_up if dist_up <= dist_down else ref_down
                elif ref_up:
                    best_ref_ctx = ref_up
                elif ref_down:
                    best_ref_ctx = ref_down

                if not best_ref_ctx: continue

                # Determine actual bottom elevation of that specific ceiling
                target_z = find_bottom_face_elevation(intersector, best_ref_ctx)
                target_pt = XYZ(location_pt.X, location_pt.Y, target_z)
        ...
        if t.GetStatus() == TransactionStatus.Started:
            t.RollBack()
        output.print_md(u"**ERROR:** Transaction failed: {}".format(e))
